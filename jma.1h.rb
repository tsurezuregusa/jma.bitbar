...
# <bitbar.version>0.9</bitbar.version>
...
def drawdots(ilist,latdeg,londeg,color,factor,opacity)
	...
	begin
		...
		for i in $typhxa..$typhxb
			...
			for j in $typhya..$typhyb
				url = "https://www.jma.go.jp/bosai/himawari/data/satimg/#{$dts}00/fd/#{$dts}00/#{$typhtype}/4/#{i}/#{j}.jpg"
		...
		ilist = satimglist.composite_layers(maplist,Magick::SoftLightCompositeOp)
		...
		$xf = satimglist.rows / ($right - $left)
		$yf = satimglist.columns / ($ymax - $ymin)
		...
		list.each do |typh|
			text = typh[4..-1].to_i.to_s
			forecast = JSON.parse(URI.open("https://www.jma.go.jp/bosai/typhoon/data/#{typh}/forecast.json").read)
			
			forecast[1]['track']['preTyphoon'].each do |c|
				ilist = drawdots(ilist,c[0],c[1],'lightskyblue',256,0.7)
			end
			forecast[1]['track']['typhoon'].each do |c|
				ilist = drawdots(ilist,c[0],c[1],'dodgerblue',256,0.7)
			end
		
			center = degpix(forecast[1]['center'][0],forecast[1]['center'][1])

			ilist = drawcircle(ilist,forecast[1]['stormWarningArea']['arc'][0][0][0],forecast[1]['stormWarningArea']['arc'][0][0][1],forecast[1]['stormWarningArea']['arc'][0][1],'orangered',0.7,nil)
			# ['arc'][0][2][0] == ??, [1] == 360?
			ilist = drawcircle(ilist,forecast[1]['center'][0],forecast[1]['center'][1],forecast[1]['galeWarningArea']['radius'],'darkorange',0.7,text)
		
			advanced = forecast.select {|x| not x['advancedHours'].nil? and x['advancedHours'] != 0}
			advanced.each do |c|
				ilist = drawdots(ilist,c['center'][0],c['center'][1],'cyan',128,0.7)
			end
			ilist = drawdots(ilist,forecast[1]['center'][0],forecast[1]['center'][1],'magenta',256,1)
		end
		# .crop(0,0,480,335,true)
		return " | refresh=true image=#{Base64.encode64(ilist.resize(0.8).to_blob).gsub(/\n/, '')}"
	rescue
		return '⚠️'
	end
end
...
typhoontarget = JSON.parse(URI.open("https://www.jma.go.jp/bosai/typhoon/data/targetTc.json", :read_timeout => 120).read).map {|x| x['tropicalCyclone']}
...
if typhoontarget.length > 0
	...
	puts "--#{typhoonimg(typhoontarget)}"
	puts typhooninfo(typhoontarget)
end
